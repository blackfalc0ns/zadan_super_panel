...
export class SuperAdminDashboardService {
  ...
  private buildSnapshot(
    ...
    return {
      ...
      headerSummaryParams: {
        ...
        mode: this.authService.isAuthenticated
          ? this.localizeModeLabel('live', lang)
          : this.localizeModeLabel('snapshot', lang)
      },
      lastUpdatedLabel: this.formatTimestamp(lang),
      systemMode: this.authService.isAuthenticated ? 'live' : 'snapshot',
      systemStatusLabelKey: this.authService.isAuthenticated
  ...
  private buildRiskQueues(filtered: DashboardSourceSnapshot): DashboardOperationalQueue[] {
    ...
    const driverHolds = filtered.drivers.filter((driver) =>
      ...
      || driver.verificationStatus === DriverVerificationStatus.Suspended
    ...
    return [
      {
        ...
        subtitleKey: this.authService.isAuthenticated
          ? 'DASHBOARD.AUDIT.SYSTEM_MODE_LIVE'
          : 'DASHBOARD.AUDIT.SYSTEM_MODE_SNAPSHOT',
        tone: this.authService.isAuthenticated ? 'success' : 'info',
  ...
  private getDriverReadiness(driver: Driver): 'ready' | 'limited' | 'blocked' {
    ...
    if (
      ...
      || driver.verificationStatus === DriverVerificationStatus.UnderReview
      || driver.verificationStatus === DriverVerificationStatus.Unverified
