...
export class SystemLogsComponent implements OnInit {
 ...
 { label: 'SYSTEM_LOGS.FILTERS.ALL', value: '' },...this.sourceOptions.map(opt => ({ label: this.sourceLabel(opt.value), value: opt.value }))
 ];
 this.filterFields[1].options = [
 { label: 'SYSTEM_LOGS.FILTERS.ALL', value: '' },...this.moduleOptions.map(opt => ({ label: this.moduleLabel(opt.value), value: opt.value }))
 ];
 }
 ...
 this.stopLivePolling();
 this.liveSubscription = interval(30000).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
 this.cdr.markForCheck();
