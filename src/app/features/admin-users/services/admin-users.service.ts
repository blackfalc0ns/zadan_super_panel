...
export class AdminUsersService {
 ...
 const vendors = this.users.filter((user) => user.assignment.vendorId).map((user) => ({
 ...
 const branches = this.users.filter((user) => user.assignment.branchId && (!vendorId || user.assignment.vendorId === vendorId)).map((user) => ({
 ...
 const duplicated = this.normalizeUser({...this.clone(existing),
 ...
 return allTargets.filter((target) =>!audienceType || target.audiences.includes(audienceType)).map(({ id, labelKey }) => ({ id, labelKey }));
 }
 ...
 this.users.filter((identity) => identity.personaType === 'super_admin_manager' && (!relatedRegion ||!identity.assignment.region || identity.assignment.region === relatedRegion)).forEach((identity) => this.pushRecipient(resolved, identity.communication.primaryEmail));
 break;
 case 'driver_account':
 ...
 return parsed.filter((item): item is AdminUserRecord => typeof item === 'object' && item!== null && 'id' in item).map((record) => this.normalizeUser(record));
 }
 ...
 return deduped.map((identity) => this.normalizeUser(identity)).sort((a, b) => a.fullName.localeCompare(b.fullName));
 }
 ...
 return driverIds.map((id) => this.driverService.getDriverSnapshotById(id)).filter((driver): driver is Driver =>!!driver).map((driver, index) => {
 ...
 const notificationEmails = this.unique([...(record.communication?.notificationEmails ?? []),...(email ? [email] : [])
 ].map((value) => value.trim().toLowerCase()).filter(Boolean));
 const escalationEmails = this.unique((record.communication?.escalationEmails ?? []).map((value) => value.trim().toLowerCase()).filter(Boolean));
 const replyTo = (record.communication?.replyTo || email).trim().toLowerCase();

 const normalized: AdminUserRecord = {...record,
 ...
 notificationEmails: this.unique((config.notificationEmails ?? []).map((value) => value.trim().toLowerCase()).filter(Boolean)),
 replyTo: (config.replyTo ?? config.primaryEmail).trim().toLowerCase(),
 escalationEmails: this.unique((config.escalationEmails ?? []).map((value) => value.trim().toLowerCase()).filter(Boolean)),
 ...
 return this.unique([
 'driver.dispatch_notifications',...(driver && driver.verificationStatus!== VerificationStatus.Verified ? ['driver.compliance_emails' as const] : []),...(driver && driver.collectionPaymentStatus!== 'critical' ? ['driver.finance_digests' as const] : [])
 ]);
 }
 case 'customer': {
 const customer = sourceRecord as CustomerDetailRecord | undefined;
 return this.unique([
 'customer.order_issue_updates',...(customer && customer.reviewState!== 'none' ? ['customer.support_escalations' as const] : []),...(customer && customer.segment!== 'watchlist' ? ['customer.marketing_opt_in' as const] : [])
 ]);
 }
 default:
 return [];
