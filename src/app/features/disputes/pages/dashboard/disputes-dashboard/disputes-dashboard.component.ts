...
export class DisputesDashboardComponent implements OnInit {
 ...
 this.adminSupportCaseRealtime.getEvents().pipe(takeUntilDestroyed(this.destroyRef)).subscribe((event) => {
 ...
 this.disputesService.approveReturnRequest(this.selectedDispute.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (dispute) => {
 ...
 this.disputesService.escalateCase(this.selectedDispute.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (dispute) => {
 ...
 this.disputesService.rejectCase(this.selectedDispute.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (dispute) => {
 ...
 this.disputesService.requestEvidence(this.selectedDispute.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (dispute) => {
 ...
 groups: Array<{ label: string; count: number }>,...labels: string[]
 ): number {
 const normalized = new Set(labels.map((label) => label.trim().toLowerCase()));
 return groups.filter((group) => normalized.has(group.label.trim().toLowerCase())).reduce((sum, group) => sum + group.count, 0);
 }
 ...
 ? 'ما قدرنا نحمّل النزاعات من الخادم. تحقق من تسجيل الدخول ثم أعد المحاولة.'
 ...
 description: this.isRtl
 ? `${stats.totalOpen} قضية مفتوحة الحين على النظام.`
