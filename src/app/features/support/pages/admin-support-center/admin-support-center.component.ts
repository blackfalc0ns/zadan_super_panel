...
export class AdminSupportCenterComponent implements OnInit {
 ...
 this.supportCasesService.approveReturnRequest(activeCase.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (updated) => {
 ...
 if (!activeCase) return;
 this.isMutatingTicket = true;
 this.supportCasesService.escalateCase(activeCase.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (updated) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 ...
 if (!activeCase) return;
 this.isMutatingTicket = true;
 this.supportCasesService.rejectCase(activeCase.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (updated) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 ...
 if (!activeCase) return;
 this.isMutatingTicket = true;
 this.supportCasesService.requestEvidence(activeCase.id, form).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (updated) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 ...
 this.isMutatingTicket = false;
 console.error(err);
 this.showCustomToast(this.describeSupportError(err, this.isRtl ? 'ما قدرنا نرسل طلب المعلومات.' : 'Failed to request additional information.'), 'error');
 }
 });
 }
 ...
 this.isMutatingTicket = true;
 this.supportCasesService.assignCase(activeCase.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (updated) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 this.applySupportCaseUpdate(updated);
 ...
 this.isMutatingTicket = false;
 console.error(err);
 this.showCustomToast(this.describeSupportError(err, this.isRtl ? 'فشل تعيين الحالة.' : 'Failed to assign support case.'), 'error');
 }
 });
 }
 ...
 subtitle: this.isRtl ? 'تقدر تضيف ملاحظة ختامية اختيارية قبل حل الحالة.' : 'You can add an optional closing note before resolving the case.',
 icon: 'done_all',
 ...
 if (ticketId) {
 this.supportService.getTicket(ticketId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((ticket) => {
 this.cdr.markForCheck();
 if (ticket) {
 ...
 this.supportService.getTicket(ticket.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((detail) => {
 this.cdr.markForCheck();
 if (detail) {
 ...
 this.isMutatingTicket = true;
 this.supportService.assignToMe(this.selectedTicket.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (ticket) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 this.setSelectedTicket(ticket, false);
 ...
 this.isMutatingTicket = true;
 this.supportService.sendMessage(this.selectedTicket.id, message).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (ticket) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 this.replyDraft = '';
 ...
 this.isMutatingTicket = true;
 this.supportCasesService.addPublicMessage(this.selectedDriverCase.id, message, 'driver').pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (driverCase) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 this.replyDraft = '';
 ...
 this.isMutatingTicket = true;
 this.supportService.updateStatus(this.selectedTicket.id, this.statusDraft).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: (ticket) => {
 this.cdr.markForCheck();
 this.isMutatingTicket = false;
 this.setSelectedTicket(ticket, false);
 ...
 return new Intl.DateTimeFormat(this.isRtl ? 'ar-SA' : 'en-US', { timeZone: 'Asia/Riyadh',
 year: 'numeric',
 month: 'short',
 day: 'numeric',
 hour: 'numeric',
