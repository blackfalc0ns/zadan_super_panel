...
export class SupportCasesService {
  ...
  private readonly casesCache = new Map<string, SupportCaseRow>();
  ...
  getStats(): Observable<AdminOrderCaseStatsResponse> {
    return this.http.get<AdminOrderCaseStatsResponse>(`${this.apiUrl}/stats`);
  }
  ...
  getSupportCases(
    ...
    const fallback = this.buildFallbackCasesPage(page, pageSize, search);
    ...
    return this.http.get<AdminOrderSupportCasesResponse>(this.apiUrl, { params }).pipe(
      map((response) => {
        const items = response.items.map((item) => this.mapSupportCase(item));
  ...
  getCasesSnapshot(): SupportCaseRow[] {
    return [...this.casesCache.values()].map((item) => ({
      ...item,
  ...
  getCaseById(id: string | null): SupportCaseRow | undefined {
    if (!id) {
      return undefined;
    }

    const normalizedId = this.normalizeId(id);
    return this.getCasesSnapshot().find((item) => this.normalizeId(item.id) === normalizedId);
  }

  private mapSupportCase(item: AdminOrderSupportCaseResponse): SupportCaseRow {
  ...
  private replaceCache(items: SupportCaseRow[]): void {
    ...
    items.forEach((item) => this.casesCache.set(item.id, item));
  }

  private upsertCache(item: SupportCaseRow): void {
    this.casesCache.set(item.id, item);
  }
  ...
  private buildFallbackCasesPage(
    page: number,
    pageSize: number,
    search?: string
