...
export class CatalogService {
 ...
 packageTypeId: item.packageTypeId || null,
 packageTypeNameAr: item.packageTypeNameAr || undefined,
 packageTypeNameEn: item.packageTypeNameEn || undefined,
 measurementValue: item.measurementValue ?? null,
 imageUrls: Array.isArray(item.imageUrls) ? item.imageUrls : (item.imageUrl ? [item.imageUrl] : undefined),
 status: item.status,
 ...
 availableSorts: this.extractArray<{ field: string; direction: 'asc' | 'desc'; label?: string }>(this.extractNestedValue(response, ['availableSorts'])).map((item) => ({ field: item.field, direction: item.direction, label: item.label })),
 ...
 const items = filtered.slice(startIndex, startIndex + pageSize).map((product) => this.cloneProduct(product));
 ...
 const totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
 const items = sorted.slice((pageNumber - 1) * pageSize, pageNumber * pageSize).map((product) => this.cloneProduct(product));
 ...
 const decoratedBase: Category = {...category,
 ...
 return categories.filter((category) => category.isActive).map((category) => ({...category,
 ...
 images: this.extractArray<string>(variant['images'] ?? variant['Images']).map((value) => typeof value === 'string' ? value.trim() : '').filter(Boolean),
 packageTypeId: readString('packageTypeId', 'PackageTypeId') || null,
 ...
 measurementUnitId: readString('measurementUnitId', 'MeasurementUnitId') || null,
 ...
 const rawCategories = this.extractArray<Record<string, unknown>>(value).map((category) => ({
