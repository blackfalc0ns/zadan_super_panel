...
export class MasterProductFormComponent implements OnInit, OnDestroy {
 ...
 return Object.entries(labels).filter(([key]) => this.productForm.get(key)?.invalid).map(([key, value]) => ({
 key,
 ...
 return [
 { value: null, labelKey: 'MASTER_PRODUCTS.GENERIC_WHITE_LABEL' },...this.availableBrands.map((brand) => ({
 value: brand.id,
 ...
 return [
 { value: null, labelKey: 'MASTER_PRODUCTS.STANDARD_UNIT' },...this.availableUnits.filter((unit) => unit.kind === 'Packaging').map((unit) => ({
 value: unit.id,
 ...
 return [
 { value: null, labelKey: 'MASTER_PRODUCTS.STANDARD_UNIT' },...this.availableUnits.filter((unit) => unit.kind === 'Measurement').map((unit) => ({
 value: unit.id,
 label: this.activeLang === 'ar' ? unit.nameAr : unit.nameEn
 }))
 ];
 ...
 const suffix = suffixSource.toLowerCase().trim().replace(/[^\u0600-\u06FFa-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
