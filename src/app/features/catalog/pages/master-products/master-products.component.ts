...
export class MasterProductsComponent implements OnInit {
 ...
 if (packageTypeField) {
 packageTypeField.options = this.units.filter((unit) => unit.kind === 'Packaging').map((unit) => ({
 value: unit.id,
 label: this.translate.currentLang === 'ar'
 ...
 measurementUnitField.options = this.units.filter((unit) => unit.kind === 'Measurement').map((unit) => ({
 value: unit.id,
 label: this.translate.currentLang === 'ar'
 ? (unit.nameAr || unit.nameEn || unit.id)
 : (unit.nameEn || unit.nameAr || unit.id)
 ...
 const leafSubcategories = flattenedCategories.filter((category) =>!!category.parentCategoryId &&!(category.subCategories?.length)
 );
 ...
 this.products.filter(item => item.id === product.id || item.variantGroupId === productGroupId).forEach(item => item.showPriceOnCard = nextValue);
 ...
 ? 'ما قدرنا نحدّث ظهور السعر الحين.'
