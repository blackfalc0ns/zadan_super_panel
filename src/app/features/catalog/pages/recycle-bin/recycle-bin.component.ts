    ...
    if (this.activeTab === 'products') {
      this.catalogService.restoreProduct(item.id).subscribe({
        ...
        this.cdr.markForCheck();
          ...
          this.errorMessage = this.activeLang === 'ar' 
            ? 'تعذر استعادة المنتج. قد يكون تصنيفه أو علامته التجارية محذوفة أولاً.'
      ...
      this.catalogService.restoreBrand(item.id).subscribe({
        ...
        this.cdr.markForCheck();
          ...
          this.errorMessage = this.activeLang === 'ar' ? 'تعذر استعادة العلامة التجارية.' : 'Failed to restore brand.';
          this.isLoading = false;
        }
      });
    } else if (this.activeTab === 'categories') {
      this.catalogService.restoreCategory(item.id).subscribe({
        ...
        this.cdr.markForCheck();
          ...
          this.errorMessage = this.activeLang === 'ar' 
            ? 'تعذر استعادة التصنيف. قد يكون التصنيف الأب محذوفاً أولاً.' 
