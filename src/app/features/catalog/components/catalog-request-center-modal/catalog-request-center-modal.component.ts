...
import { ProductRequest, ProductRequestStatus, CatalogRequestType } from '@catalog/models/catalog.domain.models';
      ...
      <div modal-body class="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
        <section class="rounded-[1.5rem] border border-slate-200/70 bg-slate-50/70 p-4">
          <div class="mb-4 flex flex-wrap items-center gap-2">
            <button
              *ngFor="let option of statusOptions"
              ...
              [ngClass]="selectedStatus === option.value
                ? 'bg-zadna-primary text-white shadow-lg shadow-zadna-primary/20'
          ...
          <div *ngIf="isLoading" class="flex min-h-[260px] items-center justify-center">
            <div class="h-10 w-10 rounded-full border-4 border-zadna-primary/15 border-t-zadna-primary animate-spin"></div>
          </div>
          ...
          <div *ngIf="!isLoading && requests.length" class="space-y-3">
            <button
              *ngFor="let request of requests"
              ...
              [ngClass]="selectedRequest?.id === request.id
                ? 'border-zadna-primary bg-white shadow-[0_10px_30px_-12px_rgba(18,124,140,0.35)]'
              ...
              <div class="flex flex-col gap-4 border-b border-slate-100 pb-5 md:flex-row md:items-start md:justify-between">
                <div class="flex items-start gap-4">
                  <div class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-[1.25rem] bg-slate-100">
                    <img
                      *ngIf="request.imageUrl; else requestFallbackIcon"
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm font-medium text-slate-500">
                      {{ activeLang === 'ar' ? request.suggestedNameEn : request.suggestedNameAr }}
                    </p>
                    <p class="mt-3 text-xs font-bold text-slate-400">
                      {{ 'CATALOG.SUBMITTED_BY' | translate }}: {{ request.vendorName || request.vendorId }}
                    </p>
                  </div>
                </div>
                ...
                <div *ngIf="request.status === 'Pending'" class="flex flex-wrap items-center gap-2">
                  <app-button
                    variant="outline"
                    size="sm"
                    customClass="!rounded-xl !border-rose-200 !text-rose-600 hover:!bg-rose-50"
                    [disabled]="isSubmitting"
                    (btnClick)="startReject()">
                  ...
                  <app-button
                    ...
                    [isLoading]="isSubmitting && pendingAction === 'approve'"
                    [disabled]="isSubmitting"
                    (btnClick)="approveSelected()">
              ...
              <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="rounded-[1.25rem] border border-slate-200/70 bg-white p-4 md:col-span-2">
                  ...
                  <div class="grid gap-3 md:grid-cols-3">
                    <div *ngIf="request.categoryPathAr || request.categoryPathEn || request.parentCategoryNameAr || request.parentCategoryNameEn">
                      <p class="text-[11px] font-bold text-slate-400">{{ 'CATALOG.CATEGORY' | translate }}</p>
                      <p class="text-sm font-black text-slate-800">
                        {{ activeLang === 'ar'
                          ? (request.categoryPathAr || request.parentCategoryNameAr || '---')
                          : (request.categoryPathEn || request.parentCategoryNameEn || '---') }}
                      </p>
                    </div>
                    <div *ngIf="request.suggestedBrandName || request.suggestedBrandNameEn">
                      ...
                      <p class="text-sm font-black text-slate-800">
                        {{ activeLang === 'ar' ? (request.suggestedBrandName || '---') : (request.suggestedBrandNameEn || request.suggestedBrandName || '---') }}
                      </p>
                    </div>
                    <div *ngIf="request.unitNameAr || request.unitNameEn">
                      <p class="text-[11px] font-bold text-slate-400">{{ 'CATALOG.UNIT' | translate }}</p>
                      <p class="text-sm font-black text-slate-800">
                        {{ activeLang === 'ar' ? (request.unitNameAr || '---') : (request.unitNameEn || request.unitNameAr || '---') }}
                      </p>
                    </div>
                ...
                <div *ngIf="showRejectForm" class="rounded-[1.25rem] border border-amber-200 bg-amber-50/70 p-4 md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows="4"
                    class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 outline-none transition-all focus:border-zadna-primary focus:ring-4 focus:ring-zadna-primary/10"
                    [placeholder]="'CATALOG.REJECTION_REASON_PLACEHOLDER' | translate"></textarea>
                  <div class="mt-4 flex flex-wrap items-center justify-end gap-2">
                    ...
                    <app-button
                      ...
                      size="sm"
                      customClass="!rounded-xl"
                      [isLoading]="isSubmitting && pendingAction === 'reject'"
...
export class CatalogRequestCenterModalComponent implements OnChanges {
  private readonly fallbackTranslations: Record<string, { ar: string; en: string }> = {
    ...
    'CATALOG.PRODUCT_REQUESTS': {
      ar: 'طلبات إضافة المنتجات',
      en: 'Product Addition Requests'
    },
    'CATALOG.BRAND_REQUESTS': {
      ar: 'طلبات إضافة العلامات التجارية',
      en: 'Brand Addition Requests'
    },
    'CATALOG.CATEGORY_REQUESTS': {
      ar: 'طلبات إضافة التصنيفات',
      en: 'Category Addition Requests'
    },
    'CATALOG.REQUESTS_EMPTY_TITLE': {
      ar: 'لا توجد طلبات مطابقة',
      en: 'No matching requests'
    },
    'CATALOG.REQUESTS_EMPTY_DESC': {
      ar: 'جرّب تغيير حالة الطلبات أو عد لاحقًا عند وصول طلبات جديدة.',
      en: 'Try changing the status filter or check again when new requests arrive.'
    },
    'CATALOG.REQUESTS_SELECT_TITLE': {
      ar: 'اختر طلبًا لعرض التفاصيل',
      en: 'Select a request to preview its details'
    },
    'CATALOG.REQUESTS_SELECT_DESC': {
      ar: 'حدد طلبًا من القائمة الجانبية لعرض البيانات الكاملة واتخاذ الإجراء المناسب.',
      en: 'Choose a request from the side list to inspect the full data and take action.'
    }
  };
  ...
  private reviewRequest(request: ProductRequest, status: 'Approved' | 'Rejected', notes?: string): void {
    const request$ = this.requestType === 'brand'
      ...
      : this.requestType === 'category'
        ? this.catalogService.reviewCategoryRequest(request.id, status, notes)
