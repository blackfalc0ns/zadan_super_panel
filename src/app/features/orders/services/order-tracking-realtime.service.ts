...
/**
 * Establishes a SignalR connection to the dedicated /hubs/order-tracking endpoint
 * and exposes streams of real-time events (driver location, status, arrival)
 * scoped to the orders the caller has subscribed to via {@link subscribe}.
 */
@Injectable({ providedIn: 'root' })
export class OrderTrackingRealtimeService {
  private static readonly signalRScriptId = 'order-tracking-signalr-sdk';
  ...
  /**
   * Subscribe the current connection to live updates for an order. Establishes
   * the underlying hub connection on the first call. Safe to call multiple
    ...
    this.connectionPromise = (async () => {
      ...
      if (!this.hubConnection) {
        ...
        this.hubConnection.onreconnected?.(async () => {
          ...
          // Re-subscribe to every order after a transparent reconnect
