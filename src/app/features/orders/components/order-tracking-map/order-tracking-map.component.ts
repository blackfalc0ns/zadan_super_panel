...
@Component({
 ...
 border-radius: 50%;
 flex-shrink: 0;
 }.legend-dot.merchant { background: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.2); }.legend-dot.customer { background: #3b82f6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.2); }.legend-dot.driver { background: #f59e0b; box-shadow: 0 0 0 3px rgba(245, 158, 11, 0.2); }.legend-label {
 font-size: 10px;
 font-weight: 700;
 color: #475569;
 }
...
export class OrderTrackingMapComponent implements AfterViewInit, OnChanges, OnDestroy {
 ...
 }).addTo(this.map).bindPopup(`<strong style="font-size:13px">${this.merchantLabel}</strong>`);
 ...
 }).addTo(this.map).bindPopup(`<strong style="font-size:13px">${this.customerLabel}</strong>`);
 bounds.push(L.latLng(pos));
 }
 ...
 }).addTo(this.map).bindPopup(`<strong style="font-size:13px">${this.driverLabel}</strong>`);
 bounds.push(L.latLng(pos));
 }
