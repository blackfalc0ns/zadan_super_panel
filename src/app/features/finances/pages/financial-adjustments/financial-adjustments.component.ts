 ...
 *ngIf="showCreateModal"
 ...
 <h3 class="text-xl font-black text-slate-900 mb-2">ما فيه سجلات مطابقة</h3>
 <p class="text-sm font-medium text-slate-500 max-w-sm">ما لقينا أي تسويات تطابق معايير الفلترة والبحث المحددة الحين.</p>
 </div>
...
export class FinancialAdjustmentsComponent implements OnInit {
 ...
 return this.adjustments.filter(a => a.direction === 'credit' && a.status === 'approved').reduce((sum, a) => sum + a.amount, 0);
 }

 get totalDebits(): number {
 return this.adjustments.filter(a => a.direction === 'debit' && a.status === 'approved').reduce((sum, a) => sum + a.amount, 0);
 }
