...
export class DashboardComponent implements OnInit {
 ...
 severity: this.resolveHighestSeverity([...(this.dashboard?.alerts.map(alert => alert.severity) ?? []),...(this.dashboard?.queues.risk.map(queue => queue.severity) ?? [])
 ]),
 ...
 this.dashboardService.getDashboardSnapshot(this.filterState, this.currentLang).pipe(take(1)).subscribe({
 next: (dashboard) => {
 ...
 const queues: DashboardQueue[] = [...(this.dashboard?.queues.live ?? []),...(this.dashboard?.queues.risk ?? [])
 ...
 this.orderSections.flatMap((section) => section.exceptions).forEach((exception) => {
 ...
 return buckets.filter(bucket => bucket.severity === 'critical' || bucket.severity === 'warning').reduce((sum, bucket) => sum + bucket.count, 0);
 }
