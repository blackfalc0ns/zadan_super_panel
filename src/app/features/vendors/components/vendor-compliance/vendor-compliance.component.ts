...
export class VendorComplianceComponent {
 ...
 return (this.vendorDetail?.reviewItems ?? []).filter((item) => item.targetType === 'field' || item.code === 'step5.logo');
 }
 ...
 */
 ...
 return this.reviewDocuments.filter((document) =>!!document.reviewedBy).sort((left, right) => (right.reviewedAtUtc || '').localeCompare(left.reviewedAtUtc || ''))[0]?.reviewedBy
 ...
 return this.localize('اكتب سبب الرفض أو ما الذي لازم على التاجر تعديله قبل إعادة الرفع...', 'Explain what must be fixed before the vendor re-uploads or before the file is rejected...');
 }
 ...
 fields.map((field) => ({
 code: field.code,
 decision: decision === 'approve' ? 'approved' : 'rejected',...(decision === 'reject' ? { reason } : {})
 }))
 )
 : of(null);
 ...
 document.isUploaded &&
 document.reviewDecision === 'pending' &&!(document.type === 'commercial' && this.isCrExpired)
 );
 }
 ...
 this.vendorDetailFacade.clearMutationError();
 this.isSubmittingDocumentDecision = true;
 this.vendorDetailFacade.approveVendorDocumentRequest(document.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: () => {
 this.cdr.markForCheck();
 ...
 this.vendorDetailFacade.clearMutationError();
 this.isSubmittingDocumentDecision = true;
 this.vendorDetailFacade.rejectVendorDocumentRequest(document.id, reason).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
 next: () => {
 this.cdr.markForCheck();
 ...
 return this.localize('ما فيه بيانات سجل مسجلة', 'No commercial registration data registered');
 ...
 return this.localize('ما فيه رقم ضريبي مسجل', 'No tax ID registered');
 ...
 if (!document?.fileUrl) {
 return this.localize('ما فيه ملف فعلي', 'No physical file');
 }
