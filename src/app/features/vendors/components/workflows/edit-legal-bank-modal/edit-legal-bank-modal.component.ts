...
export class EditLegalBankModalComponent implements OnChanges, OnInit {
 ...
 return this.text('الرقم الضريبي لازم ما يتجاوز 50 حرفًا.', 'Tax ID must not exceed 50 characters.');
 }

 if (this.draftLegalBankData.licenseNumber.trim().length > 100) {
 return this.text('رقم الرخصة لازم ما يتجاوز 100 حرف.', 'License number must not exceed 100 characters.');
 }
 ...
 return this.text('اسم البنك لازم ما يتجاوز 200 حرف.', 'Bank name must not exceed 200 characters.');
 }
 ...
 return this.text('اسم صاحب الحساب لازم ما يتجاوز 200 حرف.', 'Account holder name must not exceed 200 characters.');
 }
 ...
 return this.text('رقم IBAN لازم ما يتجاوز 34 حرفًا.', 'IBAN must not exceed 34 characters.');
 }

 if (this.draftLegalBankData.swiftCode.trim().length > 11) {
 return this.text('رمز SWIFT لازم ما يتجاوز 11 حرفًا.', 'SWIFT code must not exceed 11 characters.');
 }
 ...
 this.save.emit({...this.draftLegalBankData,
