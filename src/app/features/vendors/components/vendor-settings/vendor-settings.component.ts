...
export class VendorSettingsComponent {
 ...
 this.vendorDetailFacade.storeAvailability$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((state) => {
 this.cdr.markForCheck();
 ...
 return this.text('الإجراء الصحيح الحين هو إعادة تشغيل الحساب إلى Active.', 'The correct next action is reactivating the account to Active.');
 }
 ...
 return this.text('ما فيه إجراءات حساسة متوفرة للحالة الحالية.', 'No sensitive control action is available for the current state.');
 }
 ...
 switch (this.activeDialog) {
 case 'reset-password':
 return this.text('إعادة ضبط كلمة المرور', 'Reset password');
 case 'suspend-account':
 ...
 return note?.message?.trim() || note?.messageKey || this.text('ما فيه ملاحظة تشغيلية حديثة.', 'No recent operational note.');
 }
 ...
 if (!openDays.length) {
 return this.text('ما فيه ساعات محفوظة', 'No saved working hours');
 }
 ...
 error: () => {
 this.cdr.markForCheck();
 this.setDialogError(this.vendorDetailFacade.mutationError || this.text('ما قدرنا نحدّث كلمة المرور الحين.', 'Unable to reset vendor password right now.'));
 this.dialogSubmitting = false;
