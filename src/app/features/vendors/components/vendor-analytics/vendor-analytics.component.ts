...
export class VendorAnalyticsComponent {
  ...
  get activeWorkloadCount(): number {
    ...
    return this.analytics.orderStatusBreakdown
      .filter((s) => ['pending', 'processing', 'shipped', 'in_progress'].some(term => s.status.toLowerCase().includes(term)))
  ...
  private loadAnalytics(): void {
    ...
    this.vendorService.getVendorAnalytics(this.vendorDetail.id, this.selectedRange)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (analytics) => {
        this.cdr.markForCheck();
