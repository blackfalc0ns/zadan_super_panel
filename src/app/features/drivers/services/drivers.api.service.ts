...
export class DriverService {
 ...
 let params = new HttpParams().set('page', String(Math.max(1, page))).set('pageSize', String(Math.max(1, Math.min(pageSize, 50))));
 ...
 const params = new HttpParams().set('page', '1').set('pageSize', String(Math.max(1, Math.min(pageSize, 100))));
 ...
 let params = new HttpParams().set('page', String(Math.max(1, page))).set('pageSize', String(Math.max(1, Math.min(pageSize, 100))));
 ...
 return (value || '').toLowerCase().replace(/[^a-z\u0600-\u06ff0-9]+/g, ' ').replace(/\s+/g, ' ').trim();
 }
